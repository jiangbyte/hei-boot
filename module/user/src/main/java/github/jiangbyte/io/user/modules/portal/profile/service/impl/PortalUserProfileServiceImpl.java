package github.jiangbyte.io.user.modules.portal.profile.service.impl;
...
import github.jiangbyte.io.user.modules.portal.profile.convert.PortalUserProfileConvert;
import github.jiangbyte.io.user.modules.portal.profile.entity.PortalUserProfile;
import github.jiangbyte.io.user.modules.portal.profile.mapper.PortalUserProfileMapper;
...
import github.jiangbyte.io.user.modules.portal.profile.result.DeptIdNameResult;
import github.jiangbyte.io.user.modules.portal.profile.result.GroupIdNameResult;
...
import github.jiangbyte.io.user.modules.portal.profile.result.RoleIdNameResult;
...
/**
 * {@link PortalUserProfileService} 实现：资料持久化、头像文件处理、公开资料查询与跨模块资料读写。
 *
 * Author: Charlie
 */
@Service
...
public class PortalUserProfileServiceImpl implements PortalUserProfileService {
    ...
    public void updateProfile(ProfileUpdateParam request) {
        LoginUser loginUser = LoginHelper.requireUser();
        PortalUserProfile profile = ensureProfile(loginUser.getAccountId());
        // 映射可编辑字段；头像单独规范化对象名
    ...
    public AvatarUpdateResult uploadAvatar(MultipartFile file) {
        ...
        PortalUserProfile current = ensureProfile(loginUser.getAccountId());
    ...
    public PublicProfileResult publicProfile(String accountId) {
        PortalUserProfile profile = portalProfileMapper.selectById(accountId);
        if (profile == null) {
    ...
    public Map<String, PortalUserProfileInfo> getProfiles(Collection<String> accountIds) {
        Map<String, PortalUserProfileInfo> map = new HashMap<>();
    ...
    public void upsertProfile(PortalUserProfileInfo info) {
        ...
        PortalUserProfile profile = portalProfileMapper.selectById(info.getAccountId());
        if (profile == null) {
            // 不存在则新建
            profile = new PortalUserProfile();
    ...
    private Set<String> accountIdsByName(String name) {
        return portalProfileMapper.selectList(Wrappers.<PortalUserProfile>lambdaQuery()
                        .like(PortalUserProfile::getName, name)
                        .select(PortalUserProfile::getAccountId))
                .stream()
                .map(PortalUserProfile::getAccountId)
                .collect(Collectors.toCollection(HashSet::new));
    }

    private Set<String> accountIdsByPhone(String phone) {
        return portalProfileMapper.selectList(Wrappers.<PortalUserProfile>lambdaQuery()
                        .like(PortalUserProfile::getPhone, phone)
                        .select(PortalUserProfile::getAccountId))
                .stream()
                .map(PortalUserProfile::getAccountId)
                .collect(Collectors.toCollection(HashSet::new));
    ...
    private Set<String> accountIdsByEmail(String email) {
        return portalProfileMapper.selectList(Wrappers.<PortalUserProfile>lambdaQuery()
                        .like(PortalUserProfile::getEmail, email)
                        .select(PortalUserProfile::getAccountId))
                .stream()
                .map(PortalUserProfile::getAccountId)
                .collect(Collectors.toCollection(HashSet::new));
    ...
    private PortalUserProfile ensureProfile(String accountId) {
        // 首次访问时懒创建空资料行
        PortalUserProfile profile = portalProfileMapper.selectById(accountId);
        if (profile != null) {
            return profile;
