package github.jiangbyte.io.message.modules.feedback.service.impl;
...
import github.jiangbyte.io.message.modules.feedback.convert.MsgFeedbackConvert;
import github.jiangbyte.io.message.modules.feedback.entity.MsgFeedback;
import github.jiangbyte.io.message.modules.feedback.mapper.MsgFeedbackMapper;
...
/**
 * {@link github.jiangbyte.io.message.modules.feedback.service.FeedbackService} 实现：反馈持久化、附件规范化与提交人资料补全。
 *
 * Author: Charlie
 */
@Service
@RequiredArgsConstructor
public class FeedbackServiceImpl extends ServiceImpl<MsgFeedbackMapper, MsgFeedback> implements FeedbackService {

    private final MsgFeedbackConvert feedbackConvert;
    ...
    @ReadDataSource
    @Override
    public Page<MsgFeedback> page(MsgFeedbackPageParam param) {
        // 按状态/分类分页查询反馈
        // 补全附件与提交人展示信息
        Page<MsgFeedback> page = this.getBaseMapper().selectPage(
                new Page<>(param.getCurrent(), param.getSize()),
                Wrappers.<MsgFeedback>lambdaQuery()
                        .eq(StringUtils.hasText(param.getStatus()), MsgFeedback::getStatus, param.getStatus())
                        .eq(StringUtils.hasText(param.getCategory()), MsgFeedback::getCategory, param.getCategory())
    ...
    @ReadDataSource
    @Override
    public MsgFeedback detail(String id) {
        // 按 ID 加载反馈，不存在则 404
        // 补全附件与提交人后返回
        MsgFeedback entity = this.getById(id);
        if (entity == null) {
    ...
    @Transactional
    @Override
    public void update(MsgFeedbackEditParam param) {
        // 校验反馈存在后更新状态
        // 若有回复则记录回复人与时间
        LoginUser user = MessageAuthSupport.requireUser();
        MsgFeedback entity = this.getById(param.getId());
        if (entity == null) {
            throw new BizException(404, "反馈不存在");
        }
    ...
    @Transactional
    @Override
    public void submit(MsgFeedbackAddParam param) {
        // 转换参数并补默认标题/分类
        // 规范化附件对象名后落库为待处理
        LoginUser user = MessageAuthSupport.requireUser();
        MsgFeedback entity = feedbackConvert.toEntity(param);
    ...
    @ReadDataSource
    @Override
    public Page<MsgFeedback> myPage(MsgFeedbackPageParam param) {
        // 按当前提交人过滤并分页
        // 补全附件与提交人展示信息
        LoginUser user = MessageAuthSupport.requireUser();
        Page<MsgFeedback> page = this.getBaseMapper().selectPage(
                new Page<>(param.getCurrent(), param.getSize()),
                Wrappers.<MsgFeedback>lambdaQuery()
                        .eq(MsgFeedback::getSubmitterAccountType, MessageAuthSupport.accountType(user))
                        .eq(MsgFeedback::getSubmitterAccountId, user.getAccountId())
                        .orderByDesc(MsgFeedback::getCreatedAt));
        enrichMany(page.getRecords());
        return page;
    }
    ...
    @ReadDataSource
    @Override
    public MsgFeedback myDetail(String id) {
        // 校验反馈存在且归属当前用户
        // 补全后返回详情
        LoginUser user = MessageAuthSupport.requireUser();
        MsgFeedback entity = this.getById(id);
    ...
    private MsgFeedback enrichOne(MsgFeedback entity) {
        enrichMany(List.of(entity));
        return entity;
    }

    private void enrichMany(List<MsgFeedback> entities) {
        // 软规范化附件名并批量查文件
        // 组装附件结果后补全提交人
        ...
        for (MsgFeedback entity : entities) {
        ...
        Map<String, FileInfo> fileMap = fileApi.listByObjectNames(allNames).stream()
                .collect(Collectors.toMap(FileInfo::getObjectName, f -> f, (a, b) -> a, LinkedHashMap::new));
        for (MsgFeedback entity : entities) {
            List<MsgFeedbackAttachmentResult> attachments = new ArrayList<>();
            for (String objectName : entity.getAttachObjectNames()) {
                FileInfo file = fileMap.get(objectName);
                MsgFeedbackAttachmentResult item = new MsgFeedbackAttachmentResult();
    ...
    private void enrichSubmitters(List<MsgFeedback> entities) {
        // 按账户类型归集提交人 ID
        // 批量拉取资料并回填头像/昵称
        ...
        for (MsgFeedback entity : entities) {
            ...
            if (AccountType.ADMIN.name().equals(type)) {
                AdminUserProfileInfo profile = adminProfiles.get(entity.getSubmitterAccountId());
                if (profile == null) {
                    continue;
                }
                ...
                PortalUserProfileInfo profile = portalProfiles.get(entity.getSubmitterAccountId());
                if (profile == null) {
                    continue;
                }
                entity.setSubmitterAvatar(fileApi.resolveUrl(profile.getAvatar()));
