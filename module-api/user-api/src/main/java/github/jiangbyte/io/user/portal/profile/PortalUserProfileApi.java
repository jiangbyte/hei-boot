 ...
 * 实现见 {@code module/user} 的 {@code PortalUserProfileApiProvider}。
 *
 * Author: Charlie
 */
public interface PortalUserProfileApi {

    /** 按账号 id 获取档案；不存在时返回 null。 */
    PortalUserProfileInfo getProfile(String accountId);

    /** 批量获取档案，key 为 accountId。 */
    Map<String, PortalUserProfileInfo> getProfiles(Collection<String> accountIds);

    /** 展示名映射：优先 name，否则 nickname。 */
    Map<String, String> getDisplayNames(Collection<String> accountIds);

    /** 按 accountId 插入或更新档案。 */
    void upsertProfile(PortalUserProfileInfo info);
