package github.jiangbyte.io.user.admin.profile;
...
/**
 * 管理端用户档案跨模块 API：查询/批量查询、展示名、增删改手机邮箱及条件反查账户。
 * 实现见 {@code module/user} 的 {@code AdminUserProfileApiProvider}。
 *
 * Author: Charlie
 */
public interface AdminUserProfileApi {
    ...
    AdminUserProfileInfo getProfile(String accountId);

    /** 批量获取档案，key 为 accountId。 */
    Map<String, AdminUserProfileInfo> getProfiles(Collection<String> accountIds);

    /** 展示名映射：优先 name，否则 nickname。 */
    Map<String, String> getDisplayNames(Collection<String> accountIds);

    /** 按 accountId 插入或更新档案。 */
    void upsertProfile(AdminUserProfileInfo info);
