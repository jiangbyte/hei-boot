"""CRUD cases for key admin resources (payloads aligned with hei-fastapi / hei-gin)."""
...
def run_crud_cases(base: str, admin_tok: str, bucket: CaseBucket) -> None:
    ...
    def crud_dict() -> None:
        ...
        if not row_id:
            raise AssertionError(f"created dict not found sample={[as_string(r.get('code')) for r in recs[:10]]}")
        ...
        st, raw, ar = do_raw(
            "POST",
            f"{base}/api/v1/admin/sys/dicts/delete",
            admin_tok,
    ...
    def crud_banner() -> None:
        ...
        if not row_id:
            ...
            row_id = find_id_by_field(recs, "title", title)
        if not row_id:
            raise AssertionError(f"banner not found total={data.get('total') if data else None}")
    ...
    def crud_role() -> None:
        ...
        row_id = find_id_by_field(recs, "code", code)
        if not row_id:
            raise AssertionError(f"role not found sample={[as_string(r.get('code')) for r in recs[:10]]}")
        ubody = json.dumps(
            {
                "id": row_id,
                "code": code,
    ...
    def crud_dept() -> None:
        ...
        if not row_id:
            raise AssertionError(
                f"dept not found sample={[as_string(r.get('name')) for r in recs[:5]]}"
            )
        ubody = json.dumps(
            {
                "id": row_id,
    ...
    def crud_activity() -> None:
        code = f"E2E_ACT_{suffix}"
        body = json.dumps(
            {
                "code": code,
                ...
                "type": "NORMAL",
                "status": "DISABLED",
                "cover_url": "https://example.com/e2e.png",
                "description": "e2e activity",
                "start_at": "2026-07-19T01:00:00Z",
                "end_at": "2026-07-19T09:00:00Z",
                "max_participants": 10,
                "price": 0,
                "is_public": True,
                "need_approval": False,
                "rule_config": {"limit": {"daily": 3}},
                "extra": {"tags": ["e2e"]},
            }
        )
        st, raw, ar = do_raw("POST", f"{base}/api/v1/admin/biz/cg-test-activity/create", admin_tok, body)
        try:
            assert_biz_ok(st, ar.code)
        except AssertionError as exc:
            raise AssertionError(f"create: {exc} {truncate(raw.decode('utf-8', 'replace'), 200)}") from exc
        st, raw, ar = do_raw(
            "GET",
            f"{base}/api/v1/admin/biz/cg-test-activity/page?current=1&size=50&code={quote(code)}",
            admin_tok,
        )
        assert_biz_ok(st, ar.code)
        _, data = parse_envelope(raw)
        ...
        row_id = find_id_by_field(recs, "code", code)
        if not row_id:
            raise AssertionError("activity not found")
        st, raw, ar = do_raw(
            "GET",
            f"{base}/api/v1/admin/biz/cg-test-activity/detail?id={row_id}",
            admin_tok,
        )
        assert_biz_ok(st, ar.code)
        st, raw, ar = do_raw(
            "POST",
            f"{base}/api/v1/admin/biz/cg-test-activity/delete",
            admin_tok,
            json.dumps({"ids": [row_id]}),
        )
        assert_biz_ok(st, ar.code)
    ...
    run("crud_activity", crud_activity)
